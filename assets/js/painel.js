/* =========================================================
   Pizzaria Modelo — Painel de Pedidos (painel.html)
   Criasiteweb
...
function comecarInsistencia() {
  ...
  insistir = setInterval(() => {
    ...
    apitar(2);
    document.title = "🔔 PEDIDO ESPERANDO — Pizzaria Modelo";
  }, 20000);
}

function pararInsistencia() {
  clearInterval(insistir); insistir = null;
  document.title = "Painel de Pedidos | Pizzaria Modelo";
}

function avisarNaTela(p) {
  document.title = "🔔 PEDIDO NOVO — Pizzaria Modelo";
  setTimeout(() => { document.title = "Painel de Pedidos | Pizzaria Modelo"; }, 12000);
  if ("Notification" in window && Notification.permission === "granted") {
    try {
      new Notification("Pedido novo — Pizzaria Modelo", {
...
/* =========================================================
   ...
   ========================================================= */
const CHAVE_IMPRESSAO = "modelo:imprimir";
...
const RECADOS = {
  novo: p => `Oi ${primeiroNome(p)}! Recebemos seu pedido *#${p.numero}* aqui no Pizzaria Modelo. Já vamos preparar!`,
